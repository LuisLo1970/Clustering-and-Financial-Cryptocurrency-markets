...
nfich <- c("CRYPT_DS_18v2g3F.RData")
load(nfich)

# Dataset indices de referencia:SP500, Bono 10 y NASDAQ Composite #
# SOURCE TABLES: SP500Index.DT, SP500Index_xts, Bono10.DT, Bono10.xts, NasdaQ.DT,
#     NasdaQ.xts, Letra90.DS, Letra90.DT
nfich <- c("IndexesRef.RData")
load(nfich)

#Descriptive table of cryptocoins
nfich <- c("CoinsDescr19_18v2g3F.RData")
load(nfich)
...
BetaTabla <- merge.data.frame(CRYP_dep.RT, NasdaQ.DT, by.x ='time', by.y = 'Date', 
                              all.x = TRUE)
#Cleaning of Beta table and format change:
BetaTabla <- na.omit(BetaTabla)
BetaTabla.DT <- as.data.table(BetaTabla)
BetaTabla.DT <- BetaTabla.DT[,.(time, SYM, Rt, RtNSQ)]
setnames(BetaTabla.DT,c('Rt','RtNSQ'), c('Rt.crypt','Rt.NASDAQ'))
BetaTabla.DT$SYM <- as.factor(BetaTabla.DT$SYM)


##### Variable transformation from Quantitate to Categorical: Function definitions ############
#Quantile categories
...
betaFunc <- function(b){
        ...
        if((b>= 0.95) && (b<1.05)) return ('NASDAQlike')
...
MKCap.Perctil <- quantile(CRYPTO19.DT$mkcap18, probs = seq(0,1,length = 101),type = 8)
QMKCap <- CRYPTO19.DT[,.(PercMKCap = percFunc(mkcap18, MKCap.Perctil)),
                      by = FROMSYMBOL]
Crypto.Qualy <- merge(CRYPTO19.DT, QMKCap, all.x = TRUE)
Crypto.Qualy <- Crypto.Qualy[,.(FROMSYMBOL,Algorithm,ProofType,PercMKCap)]
names(Crypto.Qualy)<- c('SYM','Algorithm','ProofType','PercMKCap')

MCTot <- sum(CRYPTO19.DT$mkcap18)
MKTCAPShare <- CRYPTO19.DT[,.(FROMSYMBOL,MKCapRate = mkcap18/MCTot)]
...
MKTCAPShare$ClaseMKCap <- c('BC') #Marcamos cryptom de baja capitalizacion
temp <- MKTCAPShare[,.(MKCapRate >= 0.01)]
MKTCAPShare$ClaseMKCap[temp$V1]=c('AC') #Criptomonedas de alta capitalizacion (1%)
colnames(MKTCAPShare)[1] <- 'SYM'
MKTCAPShare$ClaseMKCap <- as.factor(MKTCAPShare$ClaseMKCap)

### Ordered Market cap list #############
sink('MarketCapList.txt')
MKTCAPShare
sink()


### Market cap ranking: The Top-ten
crypt.cap <- MKTCAPShare[1:10,.(SYM)]
crypt.cap

Crypto.Qualy <- merge(Crypto.Qualy,MKTCAPShare)
Crypto.Qualy <- Crypto.Qualy[,.(SYM,Algorithm, ProofType,PercMKCap,ClaseMKCap)]
...
EUrendvol <- CRYP_dep.RT[,.(vmedio = mean(volume), vmin = min(volume), vmax=max(volume), 
                            v90 = quantile(volume,probs = c(0.90)), v75 = quantile(volume,probs = c(0.75)), 
                            v25 = quantile(volume,probs = c(0.25) ), v10 = quantile(volume,probs = c(0.1)), 
...
#Sharp ratio (vs Sortino ratio)
rf <- mean(Letra90.DT$X90dayTreasuryBill/100) #Risk Free interest (interest computed over90 days U.S treasury bill)
CRYPRend_xts <- t(CRYP.timeWise.rend)
SharpR <- SharpeRatio(CRYPRend_xts, Rf=rf, FUN = 'StdDev')
EUrisk$SharpR <- SharpR[1,]
DSharpR <- sapply(EUrisk$SharpR, function(s) sharpFunc(s))

Crypto.Qualy$DecSharpR <- DSharpR

#Information Ratio
CrypRend.xts <- as.xts(CRYPRend_xts)
CrypRend.reduc.xts <- CrypRend.xts[index(NasdaQ.xts)]
Nasdaq.reduc.xts <- NasdaQ.xts[,'RtNSQC']
Nasdaq.reduc.xts <- Nasdaq.reduc.xts[-1,]

#Modigliani-Modigliani ratio (NASDAQ Composite as benchmark index)
M2 <- Modigliani(CrypRend.xts, Nasdaq.reduc.xts, Rf =  rf)
M2.Dec <- quantile(M2[1,], probs = seq(0,1,length = 11),type = 5)
DM2 <- sapply(M2[1,], function(s) decFunc(s,M2.Dec)) #Categorizamos M2

#Sortino ratio
SortRat <- SortinoRatio(CrypRend.xts, MAR = rf)
SortRat.Dec <- quantile(SortRat[1,], probs = seq(0,1,length = 11),type = 5)
DSortRat <- sapply(SortRat[1,], function(s) decFunc(s,SortRat.Dec)) #Categorizamos Sortino ratio

#M2 for Sortino is a M^2 calculated for Downside risk of Total Risk
M2Sort <- M2Sortino(CrypRend.xts, Nasdaq.reduc.xts, MAR = rf)
M2Sort.Dec <- quantile(M2Sort[1,], probs = seq(0,1,length = 11),type = 5, na.rm = TRUE)
DM2Sort <- sapply(M2Sort[1,], function(s) decFunc(s,M2Sort.Dec)) #Categorizamos M2 Sortino

#Finance ratios are incorporated to risk the quantitative table EUrisk
EUrisk$M2 <- M2[1,]
EUrisk$SortRat <- SortRat[1,]
EUrisk$M2Sort <- M2Sort[1,]

#Quantile variabls (categoric) of finance ratios are incorporated to Crypto.Qualy table
Crypto.Qualy$DecM2 <- DM2
Crypto.Qualy$DecM2Sort <- DM2Sort
Crypto.Qualy$DecSortRat <- DSortRat


##### Beta processing #################################################
## Beta computed taken NASDAQ Composite are benchmark index           #
#######################################################################
Betas <- BetaTabla.DT[,.(betaCryp=coef(lm(Rt.crypt ~ Rt.NASDAQ))[2]), by = SYM]
DBetas <- Betas[,.(DBeta=betaFunc(betaCryp)), by = SYM] #Categorical treatment

### We incorporate Beta values to Quanty EUrisk and Qualy Crypto.Qualy tables
...
#We add maturity to the Qualy table
...
vectKM1 <- c('XEM','VIA','QRL', 'DASH', 'QTUM', 'BCH','XST')
...
fig <- fig %>% add_text(text = cl1.red$SYM, color=I("grey"))
...
# Centroid 2 (detail)
vectKM2 <- c('ICE','8BIT','STAR', 'ADCN', 'GOOD', 'WAND','ITT','ZCG','PFR','ELTCOIN','FLLW','REX')
#cl.red <- cl[cl$SYM %in% vectKM1,]
...
fig <- fig %>% add_text(text = cl2.red$SYM, color=I("black"))
add_histogram2dcontour(fig, showscale=FALSE, ncontours=25) %>%
...
fig <- fig %>% add_text(text = cl3.red$SYM)
add_histogram2dcontour(fig, showscale=FALSE, ncontours=15) %>%
...
#We re-order Crypto.Qualy table
...
nfich1 <- c("ClusterResultsFocus2F.RData")
#save(k3_iter, Hist_k_results, Hist.mtx, TADPole_r ,TADPole_k, file = nfich1)
#load(nfich1)

nfich3 <- c("QualyQuant2F.RData")
#save(EUrisk, EUrendvol, MKTCAPShare, Crypto.Qualy, Crypto.Famd, file = nfich3)
#load(nfich3)
